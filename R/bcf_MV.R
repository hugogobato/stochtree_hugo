...
bcf_mv <- function(X_train, Z_train, y_train, propensity_train = NULL, rfx_group_ids_train = NULL, 
    ...
    leaf_dimension_mu_forest <- ncol(y_train)
    if (ncol(Z_train) > 1) {
        leaf_model_tau_forest <- 2
        leaf_dimension_tau_forest <- ncol(Z_train)
    } else {
        leaf_model_tau_forest <- 2
        leaf_dimension_tau_forest <- ncol(y_train)
    }
    ...
    forest_samples_mu <- createForestSamples(num_trees_mu, ncol(y_train), TRUE)
    forest_samples_tau <- createForestSamples(num_trees_tau, ncol(y_train), FALSE)
    active_forest_mu <- createForest(num_trees_mu, ncol(y_train), TRUE)
    active_forest_tau <- createForest(num_trees_tau, ncol(y_train), FALSE)
    if (include_variance_forest) {
