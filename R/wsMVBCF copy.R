...
#' @param y_train Outcome to be modeled by the ensemble. Can be a vector for univariate outcomes or a matrix for multivariate outcomes.
...
wsmvbcf <- function(X_train, Z_train, y_train, propensity_train = NULL, rfx_group_ids_train = NULL, 
    ...
    # Handle y_train as vector or matrix
    ...
    if (y_dim > 1) {
        if (probit_outcome_model) {
            stop("Probit outcome model is not supported for multivariate outcomes.")
        }
        if (sample_sigma2_leaf_mu) {
            warning("Sampling leaf scale not yet supported for multivariate leaf models, so the leaf scale parameter will not be sampled for the prognostic forest in this model.")
    ...
    if (has_prev_model) {
        previous_bcf_model <- createWSMVBCFModelFromJsonString(previous_model_json)
        ...
        if (previous_bcf_model$model_params$sample_sigma2_global) {
            # Scale is vector for multivariate, so take the mean for scalar C++ side
            previous_y_scale_scalar <- mean(previous_y_scale)
            previous_global_var_samples <- previous_bcf_model$sigma2_global_samples / (
                previous_y_scale_scalar*previous_y_scale_scalar
            )
    ...
    if (has_test) {
        if (is.null(propensity_test) && !is.null(propensity_train)) stop("Propensity score must be provided for the test set if provided for the training set")
    }
    ...
    if(sum(variable_weights_mu) > 0) variable_weights_mu <- variable_weights_mu / sum(variable_weights_mu)
    if(sum(variable_weights_tau) > 0) variable_weights_tau <- variable_weights_tau / sum(variable_weights_tau)
    if (include_variance_forest) {
        if(sum(variable_weights_variance) > 0) variable_weights_variance <- variable_weights_variance / sum(variable_weights_variance)
    }
    ...
    if (probit_outcome_model) {
        # This part assumes univariate binary outcome.
        ...
        if (standardize) {
            if (y_dim == 1) {
                ...
                y_bar_train <- colMeans(y_train)
                y_std_train <- apply(y_train, 2, sd)
            }
            y_std_train[y_std_train == 0] <- 1
        } else {
            y_bar_train <- rep(0, y_dim)
            y_std_train <- rep(1, y_dim)
        }
        ...
        if (y_dim == 1) {
            ...
            resid_train <- scale(y_train, center = y_bar_train, scale = y_std_train)
            init_mu <- colMeans(resid_train)
        }
        
        var_resid_flat <- var(as.vector(resid_train))
        
        if (is.null(sigma2_init)) sigma2_init <- 1.0*var_resid_flat
        if (is.null(variance_forest_init)) variance_forest_init <- 1.0*var_resid_flat
        if (is.null(b_leaf_mu)) b_leaf_mu <- var_resid_flat/(num_trees_mu)
        if (is.null(b_leaf_tau)) b_leaf_tau <- var_resid_flat/(2*num_trees_tau)
        
        if (is.null(sigma2_leaf_mu)) {
            sigma2_leaf_mu <- 2.0*var_resid_flat/(num_trees_mu)
        }
        if (!is.matrix(sigma2_leaf_mu)) {
            current_leaf_scale_mu <- diag(sigma2_leaf_mu, y_dim)
        } else {
            if (nrow(sigma2_leaf_mu) != y_dim || ncol(sigma2_leaf_mu) != y_dim) stop("sigma2_leaf_init for mu forest must be a scalar or a square matrix with dimension matching the outcome")
            current_leaf_scale_mu <- sigma2_leaf_mu
        }
        
        if (is.null(sigma2_leaf_tau)) {
            sigma2_leaf_tau <- var_resid_flat/(num_trees_tau)
        }
        if (!is.matrix(sigma2_leaf_tau)) {
            current_leaf_scale_tau <- diag(sigma2_leaf_tau, y_dim)
        } else {
            if (nrow(sigma2_leaf_tau) != y_dim || ncol(sigma2_leaf_tau) != y_dim) stop("sigma2_leaf_init for tau forest must be a scalar or a square matrix with dimension matching the outcome")
    ...
    leaf_model_mu_forest <- 2
    leaf_dimension_mu_forest <- y_dim
    if (ncol(Z_train) > 1) {
        leaf_model_tau_forest <- 2
        leaf_dimension_tau_forest <- y_dim
    } else {
        leaf_model_tau_forest <- 2
        leaf_dimension_tau_forest <- y_dim
    }
    ...
    forest_samples_mu <- createForestSamples(num_trees_mu, leaf_dimension_mu_forest, TRUE)
    forest_samples_tau <- createForestSamples(num_trees_tau, leaf_dimension_tau_forest, FALSE)
    active_forest_mu <- createForest(num_trees_mu, leaf_dimension_mu_forest, TRUE)
    active_forest_tau <- createForest(num_trees_tau, leaf_dimension_tau_forest, FALSE)
    if (include_variance_forest) {
    ...
    active_forest_mu$prepare_for_sampler(forest_dataset_train, outcome_train, forest_model_mu, 2, init_mu)
    ...
    if(y_dim == 1) init_tau <- 0. else init_tau <- rep(0, y_dim)
    active_forest_tau$prepare_for_sampler(forest_dataset_train, outcome_train, forest_model_tau, 2, init_tau)
    ...
    # ... (sampling loops are complex, but the main change is to remove muhat_train_raw caching)
    # The adaptive coding part should work with matrix math.
    
    # ... After sampling loops ...
    
    # Forest predictions
    mu_hat_train_unscaled <- forest_samples_mu$predict_raw(forest_dataset_train)
    tau_hat_train_unscaled <- forest_samples_tau$predict_raw(forest_dataset_train)

    if (adaptive_coding) {
        b_diff <- b_1_samples - b_0_samples
        tau_hat_train <- sweep(tau_hat_train_unscaled, 3, b_diff, "*")
    } else {
        tau_hat_train <- tau_hat_train_unscaled
    }

    if (y_dim == 1) {
        mu_hat_train <- mu_hat_train_unscaled * y_std_train + y_bar_train
        tau_hat_train <- tau_hat_train * y_std_train
    } else {
        mu_hat_train <- array(NA, dim = dim(mu_hat_train_unscaled))
        tau_hat_train_scaled <- array(NA, dim = dim(tau_hat_train))
        for (s in 1:dim(mu_hat_train_unscaled)[3]) {
            mu_hat_train[,,s] <- sweep(mu_hat_train_unscaled[,,s], 2, y_std_train, "*")
            mu_hat_train[,,s] <- sweep(mu_hat_train[,,s], 2, y_bar_train, "+")
            tau_hat_train_scaled[,,s] <- sweep(tau_hat_train[,,s], 2, y_std_train, "*")
        }
        tau_hat_train <- tau_hat_train_scaled
    }

    y_hat_train <- mu_hat_train + sweep(tau_hat_train, 1, as.numeric(Z_train), "*")

    if (has_test) {
        mu_hat_test_unscaled <- forest_samples_mu$predict_raw(forest_dataset_test)
        tau_hat_test_unscaled <- forest_samples_tau$predict_raw(forest_dataset_test)
        if (adaptive_coding) {
            b_diff <- b_1_samples - b_0_samples
            tau_hat_test <- sweep(tau_hat_test_unscaled, 3, b_diff, "*")
        } else {
            tau_hat_test <- tau_hat_test_unscaled
        }

        if (y_dim == 1) {
            mu_hat_test <- mu_hat_test_unscaled * y_std_train + y_bar_train
            tau_hat_test <- tau_hat_test * y_std_train
        } else {
            mu_hat_test <- array(NA, dim = dim(mu_hat_test_unscaled))
            tau_hat_test_scaled <- array(NA, dim = dim(tau_hat_test))
            for (s in 1:dim(mu_hat_test_unscaled)[3]) {
                mu_hat_test[,,s] <- sweep(mu_hat_test_unscaled[,,s], 2, y_std_train, "*")
                mu_hat_test[,,s] <- sweep(mu_hat_test[,,s], 2, y_bar_train, "+")
                tau_hat_test_scaled[,,s] <- sweep(tau_hat_test[,,s], 2, y_std_train, "*")
            }
            tau_hat_test <- tau_hat_test_scaled
        }
        y_hat_test <- mu_hat_test + sweep(tau_hat_test, 1, as.numeric(Z_test), "*")
    }
    ...
    if (has_rfx) {
        rfx_preds_train <- rfx_samples$predict(rfx_group_ids_train, rfx_basis_train)
        rfx_preds_train_scaled <- rfx_preds_train * mean(y_std_train)
        y_hat_train <- y_hat_train + array(rfx_preds_train_scaled, dim=dim(y_hat_train))
    }
    if ((has_rfx_test) && (has_test)) {
        rfx_preds_test <- rfx_samples$predict(rfx_group_ids_test, rfx_basis_test)
        rfx_preds_test_scaled <- rfx_preds_test * mean(y_std_train)
        y_hat_test <- y_hat_test + array(rfx_preds_test_scaled, dim=dim(y_hat_test))
    }
    ...
    model_params <- list(
        ...
        "num_variance_covariates" = ifelse(include_variance_forest, sum(variable_weights_variance > 0), 0),
    ...
    # ... (rest of the return list construction) ...
    class(result) <- "wsmvbcfmodel"
