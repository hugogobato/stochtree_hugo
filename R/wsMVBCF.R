...
bcf <- function(X_train, Z_train, y_train, propensity_train = NULL, rfx_group_ids_train = NULL, 
    ...
    if ((is.null(dim(y_train))) && (!is.null(y_train))) {
        y_train <- as.matrix(as.numeric(y_train))
    }
    ...
    if (probit_outcome_model) {
        ...
        if (standardize) {
    # Calculate the mean for each column (feature) of y_train
    y_bar_train <- colMeans(y_train)

    # Calculate the standard deviation for each column (feature) of y_train
    # The 'sd' function by default works on columns of a matrix.
    y_std_train <- apply(y_train, 2, sd) # Explicitly apply sd to each column

       } else {
    num_features <- ncol(y_train) # Get the number of features (dimensions of y_i)
    y_bar_train <- rep(0, num_features)
    y_std_train <- rep(1, num_features)
    }
